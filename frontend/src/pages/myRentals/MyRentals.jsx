...
const MyRentals = ({loggedInUser}) => {
    ...
    return (
        <main className="main-content">
            ...
            <p className="page-subtitle">Pregled svih bicikla koje ste iznajmili.</p>
                ...
                {iznajmljivanja.map((iznajmljivanje) => (
                    <article className="rental-card" key={iznajmljivanje.id}>
                        <div className="rental-card-art">
                            <img src={getBiciklImage(iznajmljivanje.kategorijaNaziv)} alt={iznajmljivanje.kategorijaNaziv || "Bicikl"}/>
                        </div>
                        <div className="rental-card-body">
                            <p className="manufacturer">{iznajmljivanje.kategorijaNaziv || "Bicikl"}</p>
                            <h2>
                                {iznajmljivanje.rezolucija
                                    ? `${iznajmljivanje.proizvodjacNaziv} · ${iznajmljivanje.rezolucija}`
                                    : iznajmljivanje.proizvodjacNaziv}
                            </h2>
                            {iznajmljivanje.opis && <p className="bicikl-card-desc">{iznajmljivanje.opis}</p>}
